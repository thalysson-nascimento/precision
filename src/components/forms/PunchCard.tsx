...
export const PunchCard: React.FC = () => {
  ...
  const loadRecords = async () => {
    try {
      setIsLoading(true);
      const data = await timeRecordService.getTodayRecords();
  ...
  const handlePunch = async () => {
    ...
    try {
      const res = await timeRecordService.confirmEligibleRecords();
      if (res.success) {
  ...
  const handleHistoryDayClick = (day: HistoryDayData) => {
