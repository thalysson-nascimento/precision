...
import { GetReportsResponse, ReportMonthData, timeRecordService } from '@/services/time-record.service';
...
export const ReportsScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchReports = async () => {
      try {
        setIsLoading(true);
        const res = await timeRecordService.getMonthlyReports();
        setData(res);
      } catch (err) {
        console.error(err);
  ...
  const getMaxHoursValue = (reports: ReportMonthData[]): number => {
