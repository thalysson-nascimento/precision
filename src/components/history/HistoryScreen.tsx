...
import { timeRecordService, EmployeeData, HistoryDayData } from '@/services/time-record.service';
...
export const HistoryScreen: React.FC = () => {
  ...
  const loadData = async () => {
    try {
      setIsLoading(true);
      const res = await timeRecordService.getTodayRecords();
  ...
  const getGroupedMonths = (daysList: HistoryDayData[]) => {
    const grouped: Record<string, { monthName: string; year: string; monthIndex: number; days: HistoryDayData[] }> = {};
  ...
  const handleDayClick = (day: HistoryDayData) => {
    setModalState({
      isOpen: true,
      date: day.date,
      times: day.times,
