...
import { timeRecordService, EmployeeData } from '@/services/time-record.service';
...
export const DayAdjustModal: React.FC<DayAdjustModalProps> = ({
  ...
  const handleSaveTime = async () => {
    ...
    try {
      const res = await timeRecordService.updateRecordTime(editingType, formattedTime, date);
      if (res.success) {
        onSaveSuccess();
  ...
  const handleBatchConfirm = async () => {
    ...
    try {
      const res = await timeRecordService.confirmEligibleRecords(date);
      if (res.success) {
        onSaveSuccess();
      }
    } catch (e) {
