...
export default function NewEmployeePage() {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      if (!isSuperAdmin && meData.companyId) {
        setSelectedCompanyId(meData.companyId);
      }

      // 2. Fetch companies if SuperAdmin
      if (isSuperAdmin) {
      ...
      // 4. Fetch all employees (to pick team leaders as managers)
      ...
      // 5. Fetch dashboard metrics for header
  ...
  const filteredManagers = allEmployees.filter(
    e => e.companyId === selectedCompanyId && e.isTeamLeader
  );
        ...
        <main className="flex-1 ml-0 md:ml-64 flex flex-col h-screen overflow-hidden bg-background">
          ...
          <div className="flex-1 overflow-y-auto p-container-margin md:p-xl">
            <div className="max-w-[800px] mx-auto flex flex-col gap-xl">
              ...
              {/* Title */}
              <div>
                <h1 className="font-headline-lg text-headline-lg text-on-surface">Adicionar Colaborador</h1>
                <p className="font-body-lg text-body-lg text-on-surface-variant mt-xs">Cadastre um novo funcionário e defina suas jornadas e cargos.</p>
              </div>
              ...
              {loading ? (
                ...
                <form onSubmit={handleSubmit} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-xl shadow-sm space-y-xl">
                  ...
                  {/* Organização e Escopo de Multitenancy */}
                  <div className="space-y-md">
                    <h2 className="text-body-lg font-bold text-primary border-b border-outline-variant pb-xs flex items-center gap-xs">
                    ...
                    <div className="flex items-center justify-between bg-surface-container-low p-md rounded-xl border border-outline-variant/60">
                      <div className="space-y-[2px]">
                        ...
                        <p className="text-body-sm text-on-surface-variant/80">Se ativo, ele poderá ser selecionado como responsável direto por outros colaboradores.</p>
                      </div>
